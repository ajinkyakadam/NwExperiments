...
try:
    ...
    log_fileName = open('/tmp/router3log', 'w')
    ...
    if index1 == 0:
        ...
        if index2 == 0:
            ...
            child.sendline('configure terminal')
            child.expect('.+#')
            child.sendline('enable password router3')
            child.expect('.+#')

        child.sendline('service password-encryption')
        child.expect('.+#')
        child.sendline('hostname router3')
        child.expect('.+#')
        child.sendline('interface eth1')
        child.expect('.+#')
        child.sendline('description Interface connected to router2')
        child.expect('.+#')
        child.sendline('ip address 192.168.1.10/30')
        child.expect('.+#')
        child.sendline('interface eth2')
        child.expect('.+#')
        child.sendline('description Interface connected to router4')
        child.expect('.+#')
        child.sendline('ip address 192.168.1.14/30')
        child.expect('[\w()-]#')
        child.sendline('exit')
        child.expect('.+#')
        child.sendline('exit')
