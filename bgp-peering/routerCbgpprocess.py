...
try:
    ...
    log_fileName = open('/tmp/routerCbgplog', 'w')
    ...
    if index1 == 0:
        ...
        child.sendline('router bgp 65200')
        child.expect('.+#')
        child.sendline('network 10.1.3.0 mask 255.255.255.252')
        child.expect('.+#')
        child.sendline('neighbor 10.1.3.1 remote-as 65100')
        child.expect('.+#')
        child.sendline('neighbor 10.1.4.2 remote-as 65200')
        child.expect('.+#')
        child.sendline('neighbor 10.1.4.2 next-hop-self')
        child.expect('.+#')
        child.sendline('end')
        child.expect('.+#')
