...
try:
    ...
    log_fileName = open('/tmp/routerAlog', 'w')
    ...
    if index1 == 0:
        ...
        ## get interface name
        iface1 = re.search(r'e\w+\d',os.popen('ip addr | grep "10.1.1.1"').read()).group()
        iface2 = re.search(r'e\w+\d',os.popen('ip addr | grep "10.1.2.1"').read()).group()
        child.sendline('interface '+iface1)
        child.expect('.+#')
        child.sendline('description Interface connected to H1')
        child.expect('.+#')
        child.sendline('ip address 10.1.1.1/24')
        child.expect('.+#')
        child.sendline('interface '+iface2)
        child.expect('.+#')
        child.sendline('description Interface connected to routerB')
        child.expect('.+#')
        child.sendline('ip address 10.1.2.1/24')
        child.expect('[\w()-]#')
        child.sendline('exit')
        child.expect('.+#')
        child.sendline('exit')
