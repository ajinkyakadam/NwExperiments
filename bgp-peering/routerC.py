...
try:
    ...
    log_fileName = open('/tmp/routerClog', 'w')
    ...
    if index1 == 0:
        ...
        if index2 == 0:
            ...
            child.sendline('configure terminal')
            child.expect('.+#')
            child.sendline('enable password routerC')
            child.expect('.+#')

        child.sendline('service password-encryption')
        child.expect('.+#')
        child.sendline('hostname routerC')
        child.expect('.+#')

        ## get interface name
        iface1 = re.search(r'e\w+\d',os.popen('ip addr | grep "10.1.3.2"').read()).group()
        iface2 = re.search(r'e\w+\d',os.popen('ip addr | grep "10.1.4.1"').read()).group()
        child.sendline('interface '+iface1)
        child.expect('.+#')
        child.sendline('description Interface connected to router B which is a eBGP peer')
        child.expect('.+#')
        child.sendline('ip address 10.1.3.2/30')
        child.expect('.+#')
        child.sendline('interface '+iface2)
        child.expect('.+#')
        child.sendline('description Interface connected to router D which is a iBGP peer')
        child.expect('.+#')
        child.sendline('ip address 10.1.4.1/30')
        child.expect('[\w()-]#')
        child.sendline('exit')
        child.expect('.+#')
        child.sendline('exit')
