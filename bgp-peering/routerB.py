...
try:
    ...
    log_fileName = open('/tmp/routerBlog', 'w')
    ...
    if index1 == 0:
        ...
        ## get interface name
        iface1 = re.search(r'e\w+\d',os.popen('ip addr | grep "10.1.2.2"').read()).group()
        iface2 = re.search(r'e\w+\d',os.popen('ip addr | grep "10.1.3.1"').read()).group()
        child.sendline('interface '+iface1)
        child.expect('.+#')
        child.sendline('description Interface connected to router A which is a iBGP peer')
        child.expect('.+#')
        child.sendline('ip address 10.1.2.2/30')
        child.expect('.+#')
        child.sendline('interface '+iface2)
        child.expect('.+#')
        child.sendline('description Interface connected to router C which is a eBGP peer')
        child.expect('.+#')
        child.sendline('ip address 10.1.3.1/30')
        child.expect('[\w()-]#')
        child.sendline('exit')
        child.expect('.+#')
        child.sendline('exit')
